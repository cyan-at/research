classdef extractorHOG < handle
    ...
    methods
        function self = extractorHOG(pathStructArray, gridSpacing, patchSize)
            if ~exist('pathStructArray','var'), error('extractorHOG: pathStructArray missing'); end;
            if (size(pathStructArray,2) == 0), error('extractorHOG: pathStructArray is empty'); end;
