rootSrcPath = 	'/mnt/neocortex/scratch/3dproject/data/KITTI/kitti_softmax_truck';
rootSavePath =  '/mnt/neocortex/scratch/jumpbot/data/3dproject/9_18_12/truckpc';
...
imgTrainCarPath = sprintf('%s/truck', imgTrainPath);
imgTrainNegPath = sprintf('%s/not', imgTrainPath);
imgTestCarPath = sprintf('%s/truck', imgTestPath);
...
siftFeatTrainCarPath = sprintf('%s/truck', siftFeatTrainPath);
siftFeatTrainNegPath = sprintf('%s/not', siftFeatTrainPath);
siftFeatTestCarPath = sprintf('%s/truck', siftFeatTestPath);
...
hogFeatTrainCarPath = sprintf('%s/truck', hogFeatTrainPath);
hogFeatTrainNegPath = sprintf('%s/not', hogFeatTrainPath);
hogFeatTestCarPath = sprintf('%s/truck', hogFeatTestPath);
...
matTrainCarPath = sprintf('%s/truck', matTrainPath);
matTrainNegPath = sprintf('%s/not', matTrainPath);
matTestCarPath = sprintf('%s/truck', matTestPath);
...
siFeatTrainCarPath = sprintf('%s/truck', siFeatTrainPath);
siFeatTrainNegPath = sprintf('%s/not', siFeatTrainPath);
siFeatTestCarPath = sprintf('%s/truck', siFeatTestPath);
