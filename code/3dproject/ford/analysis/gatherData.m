function [neg_scores,pos_scores] = gatherData(interv,numSamples,saveName)
% numSamples = 200;
% interv = 5;
%imports
...
% three_dir = 'train_plus_interv5';
target_dir = sprintf('%s/%s',pwd,saveName);
ensure(target_dir);
...
pos_count = 0;
for i = 1:interv:length(selection)
    fprintf('%d scenes left for positives\n',length(selection)-i);
    ...
    pos_labels_2D = [pos_labels_2D,label2D];
    pos_labels_3D = [pos_labels_3D,label3D];
...
neg_count = 0;
for i = 1:interv:length(selection)
    fprintf('%d scenes left for negatives\n',length(selection)-i);
    desc = cell2mat(description(selection(i)));
    %parse out the scene, cam
    x = strsplit(desc,'_');
    scene = cell2mat(x(1));
    ...
    neg_labels_2D = [neg_labels_2D,label2D];
    neg_labels_3D = [neg_labels_3D,label3D];
...
neg_scores.ltwo = neg_labels_2D;
neg_scores.lthree = neg_labels_3D;
...
pos_scores.ltwo = pos_labels_2D;
pos_scores.lthree = pos_labels_3D;
