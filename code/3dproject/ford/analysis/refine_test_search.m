function ap = refine_test_search(rbfmodel,rbf_name,exp_desc,iteration_step,xfeature,yfeature,opt_standardize)
...
for j=1:iteration_step:length(d)
    ...
    %get all 3D seeds
    labelIdx = 12;
    seeds3D = unique(data(:,labelIdx));
    
    for seedi = 1:length(seeds3D)
        labl = seeds3D(seedi);
        in_seed_idx = find(data(:,labelIdx)==labl);
        in_seed = data(in_seed_idx,:);
        
    end
    
%     for q = 1:size(bbox,1)
%         %use every bounding box as a seed
%         seed = bbox(q,:);
%         [generatedBoxes,goodIdx] = exploreNeighbors(seed,img,data);
%     end
