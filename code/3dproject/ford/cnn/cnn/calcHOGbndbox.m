function [ hog_bndbox ] = calcHOGbndbox( img, sbin, bb )
%UNTITLED Summary of this function goes here
%   Detailed explanation goes here

hog_bndbox = zeros(size(bb,1),size(bb,2));
for i=1:size(bb,1)
    %sbin = 8;
    bbox = bb(i,:)+sbin;
...
%     hog_bndbox(i,:) = [gridX(idx) gridY(idx) gridX(idx2) gridY(idx2)];
    ...
    hog_bndbox(i,:) = [gridX(idx) gridY(idx) gridX(idx)+round((bbox(3)-bbox(1))/sbin) gridY(idx)+round((bbox(4)-bbox(2))/sbin)];
