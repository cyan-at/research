function model = train_cnn_oak(train_IMAGES,val_IMAGES,train_gt,val_gt,overlap,filter_size,numChannels,lambda,numIter,optConv)
...
if length(lambda) > 1
    ...
    for i=1:length(lambda)
        ...
        train_cost{i} = batch_obj_func(train_IMAGES,Y,models{i}.W,models{i}.b,numChannels,optConv,[]);
        train_cost_l{i} = batch_obj_func(train_IMAGES,Y,models{i}.W,models{i}.b,numChannels,optConv,lambda(i));
        pred_bbox = predict_cnn(train_IMAGES,filter_size,length(train_IMAGES),models{i},'J');
        train_acc{i} = eval_cnn(pred_bbox,train_gt,0.5,'ap');
        ...
        pred_bbox = predict_cnn(val_IMAGES,filter_size,length(val_IMAGES),models{i},'J');
