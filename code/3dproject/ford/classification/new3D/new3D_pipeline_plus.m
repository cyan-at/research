    ...
    try
        ...
        for j = 1:length(extractor.pathStructArray)
            ...
            if (strcmp(parameters.mode,'train'))
                ...
                imagesPerClass = images_per_class;
                batchSize = 100;
                %we want to collect up 50 128 x 1 vectors x 100 images
                %so our features matrix will be 128x20000
                reuse = false;
                features = new3D_collectFeatures(srcPath, imagesPerClass, batchSize, reuse);
...
% [prec, tpr, fpr, thresh] = prec_rec(raw_score_test, testLabels, varargin);
