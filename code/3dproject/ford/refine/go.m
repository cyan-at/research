...
output_dir = '/mnt/neocortex/scratch/jumpbot/data/3dproject/withlabels/16_coverage/';
ensure(output_dir);
...
total = 0;
totalpunished = 0; totaltightened = 0;
...
for i = 1:length(fs)
    ...
    if (~isempty(cnnDetections))
        [newDetections, punished,tightened] = overlapCNNv2(workingPath,cnnDetections,PARAM,...
            nesting,punish,coverage...
            );
        totalpunished = totalpunished + punished;
        totaltightened = totaltightened + tightened;
...
fprintf('total detections tightened: %d\n', totaltightened);
%copy over map file
